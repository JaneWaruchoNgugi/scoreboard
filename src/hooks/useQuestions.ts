...
import { subscribeToQuestions } from "../firebase";
...
// ── ROUND 2 ── categories ──────────────────────────────────────────────────
export const ROUND_2_CATEGORIES: Category[] = [
    // {
    //     id: 1,
...
export function useQuestions() {
    ...
    useEffect(() => {
        return subscribeToQuestions((raw) => {
            if (!raw) return;
    ...
    return {
        ...data,
        totalRound1: data.round1.length,
        totalRound2Categories: data.round2.length,
        totalRound3: data.round3.length,
    };
}
